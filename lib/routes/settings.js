(function (exports) {
    ...
    function readWRTConfig(callback) {
      ...
      fs.readFile((path.join(webinosPath, 'wrt/webinos_runtime.json')), function (err, data) {
        if (err) {
          console.log(err);
        } else {
    ...
    exports.saveSettings = function (req, res) {        
      ...
      readPZPConfig(function (pzpConfig) {
        ...
        fs.writeFile((path.join(webinosPath, 'wrt/webinos_pzp.json')), JSON.stringify(pzpConfig, null, ' '), function (err) {
          if (err) {
