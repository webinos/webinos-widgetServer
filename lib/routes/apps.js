(function(exports) {
  ...
  function writeSessionFile(sesh) {
    ...
    var sessionsFolder = path.join(webinosPath,'wrt/sessions');
    console.log(sessionsFolder);
    if (!path.existsSync(sessionsFolder)) {
        fs.mkdirSync(sessionsFolder);
    }
    var seshFile = path.join(webinosPath,'wrt/sessions',sesh.id + ".json");
    ...
    if (installId.length > 0) {
      ...
      // Widget not installed.
        ...
        if (signedOnly && result.status != wm.WidgetConfig.STATUS_VALID) {
          ...
          // startFileProtocol can be null or undefined and indicates local file
          // console.log("apps.boot - startFileProtocol:" + startFileProtocol)
          if (typeof startFileProtocol === "undefined" || startFileProtocol == null) {
          ...
          // Redirect to widget start file.
            if (useWGTProtocol) {
              res.redirect('wgt://' + installId + "/" + nextSessionId + "/" + startFile);
            } else {
