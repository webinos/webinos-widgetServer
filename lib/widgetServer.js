(function (exports) {
  ...
  exports.start = function (signedWidgetOnly, enforceWidgetCSP, useWidgetProtocol, pzpWebSocketPort, brandingFile, callback) {
    ...
    // Add a static path to PZP web-server root so that webinos.js can be picked up.
    // This can be re-visited when the two servers are merged.
//    app.use(express.static(path.join(__dirname,'../../../web_root')));
    // [cbot] no need to expose pzp's web_root from widgetServer, we just need the webinos.js file.
    var webinosPath = require("webinos-utilities").webinosPath.webinosPath();
    var webinosJSPath = path.join(webinosPath, "wrt", "webinos.js");
    /**
     ...
     */
    app.get("/webinos.js", function (req, res) {
      var stream = fs.createReadStream(webinosJSPath);
    ...
    var branding = JSON.parse(fs.readFileSync(path.join(__dirname,brandingFile + "_branding.json")));
    ...
    apps.setUseWGTProtocol(useWidgetProtocol);
